...
crane copy "projects.registry.vmware.com/tanzu_observability_keights_saas/kubernetes-operator:${OPERATOR_ALPHA_TAG}" "caapm/kubernetes-operator:${OPERATOR_VERSION}" || true
crane validate --remote "caapm/kubernetes-operator:${OPERATOR_VERSION}"
crane copy "projects.registry.vmware.com/tanzu_observability_keights_saas/kubernetes-collector:${COLLECTOR_ALPHA_TAG}" "caapm/kubernetes-collector:${COLLECTOR_VERSION}" || true
crane validate --remote "caapm/kubernetes-collector:${COLLECTOR_VERSION}"
...
sed -i.bak "s#image: ${OPERATOR_ALPHA_IMAGE}#image: caapm/kubernetes-operator:${OPERATOR_VERSION}#g" ${REPO_ROOT}/operator/dev-internal/deploy/wavefront-operator.yaml
